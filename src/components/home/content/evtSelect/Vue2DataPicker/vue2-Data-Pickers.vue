    ...
    <transition name="fade">
      <div class="date-box" v-if="dateBoxFlag">
        ...
        <div class="day-screen">
          <div>
            <span v-for="(week,index) in week">{{ week }}</span>
          </div>
          <div @click="selectDay">
            <span v-for="(day,index) in previousMonth" class="previousMonth"> {{ day }} </span>
            <span v-for="(day,index) in monthDay[month - 1]" :class="isActive(day)" class="currentMonth">{{ day }}</span>
            <span v-for="(day,index) in nextMonth" class="nextMonth">{{ day }}</span>
          </div>
        </div>
      </div>
    </transition>
